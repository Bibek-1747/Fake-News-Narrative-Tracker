...
### 4. Exploring the Dataset (EDA)
Once data is collected, you can generate a CSV of the processed dataset and run Exploratory Data Analysis (EDA) on it.
```bash
python backend/export_data.py
python backend/eda.py
```
This will generate:
- `backend/processed_news_dataset.csv`: The finalized clean dataset ready for submission.
- `backend/eda_outputs/`: A folder containing visual charts (wordclouds, region distribution) for the data analysis report.
